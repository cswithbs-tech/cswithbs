    ...
    node.marks.forEach((mark) => {
      switch (mark.type) {
        ...
        case "code":
          ...
          element = <code className={mark.attrs?.class}>{element}</code>;
          break;
        case "link":
        ...
        case "highlight":
          element = (
            <mark
              ...
              data-color={mark.attrs?.color} // Also save as data attr just in case
            >
              {element}
            </mark>
          );
...
import { CodeBlock } from "./NoteCodeBlock"; // Ensure this import path is correct based on file structure
...
const sanitizeClass = (cls: string | undefined): string | undefined => {
  return cls; // Return the exact classes saved by the editor to preserve custom styling
};
...
const NodeRenderer = ({ node, isFirst, pageTitle }: { node: JSONContent, isFirst?: boolean, pageTitle?: string }) => {
  switch (node.type) {
    ...
    case "paragraph":
      return (
        <p 
          className={sanitizeClass(node.attrs?.class)}
    ...
    case "heading":
      ...
      return (
        <Tag 
          id={slug} 
          className={`group flex items-center ${sanitizeClass(node.attrs?.class) || ''}`.trim()}
          style={{ textAlign: node.attrs?.textAlign }}
        >
          {node.content?.map((c, i) =>
            c.type === "text" ? (
    ...
    case "image":
      ...
      return (
        <figure style={figureStyle} className={sanitizeClass(node.attrs?.class)}>
          <img
            src={node.attrs?.src}
            alt={node.attrs?.alt || ""}
    ...
    case "tableRow":
      return (
        <tr className={sanitizeClass(node.attrs?.class)}>
          {node.content?.map((c, i) => (
            <NodeRenderer key={i} node={c} />
          ))}
        </tr>
    ...
    case "tableHeader":
      return (
        <th 
          ...
          style={{ 
            backgroundColor: node.attrs?.colwidth ? 'rgba(255,255,255,0.05)' : undefined,
            width: node.attrs?.colwidth ? `${node.attrs.colwidth[0]}px` : undefined 
          }}
        >
          {node.content?.map((c, i) => (
            <NodeRenderer key={i} node={c} />
