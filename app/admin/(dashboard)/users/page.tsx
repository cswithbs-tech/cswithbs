...
async function getUsers(
  ...
  // Get post counts for filtered users
  ...
  // Get comment counts for filtered users
                  ...
                  >
                    <td className="px-6 py-4 align-middle">
                      <div className="flex items-center gap-3">
                        {user.image ? (
                          <div className="relative h-10 w-10 rounded-full overflow-hidden ring-2 ring-white/5 shadow-sm">
                    ...
                    <td className="px-6 py-4 align-middle">
                      <div className="flex flex-wrap gap-2">
                        {(() => {
                          // Support legacy array. Filter out PREMIUM, replace EDITOR with WRITER, remove duplicates.
                          const rawRoles = user.roles || [];
                          let mappedRoles = rawRoles
                            .filter((r: string) => r && r.toUpperCase() !== 'PREMIUM')
                          ...
                          return validRoles.map((role: any) => (
                            <span
                              key={role}
                              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border tracking-wider ${
                                role === 'SUPER_ADMIN'
                    ...
                    <td className="px-6 py-4 align-middle">
                      {user.isPremium || user.roles?.includes('PREMIUM') || user.roles?.includes('SUPER_ADMIN') ? (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border tracking-wider bg-amber-500/10 text-amber-400 border-amber-500/20">
                          PREMIUM
                        </span>
                      ) : (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border tracking-wider bg-zinc-800/50 text-zinc-500 border-zinc-700/50">
                          FREE
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 align-middle text-right">
                      <UserRowActions
                        ...
                        isPremium={user.isPremium || user.roles?.includes('PREMIUM')}
                        userName={user.name}
                      />
                    </td>
                  </tr>
