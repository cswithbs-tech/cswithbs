...
export default function SubjectCurriculumPage() {
  ...
  const [resourceForm, setResourceForm] = useState({ title: "", type: "PDF", url: "", isPremium: false });
  ...
  const handleAddResource = async (e: React.FormEvent, chapterId: string) => {
    e.preventDefault();
    if (!resourceForm.title.trim()) return showToast("Resource title is required", "error");
    if (!resourceForm.url.trim() && !resourceFile) return showToast("File or Link is required", "error");
    ...
    try {
      ...
      const newResource = {
        ...
        url: finalUrl,
        isPremium: resourceForm.isPremium
      };
      
      const updatedResources = [...(chapter.resources || []), newResource];
      
      const res = await fetch(`/api/writers-hub/chapters/${chapterId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      setChapters(prev => prev.map(c => c._id === chapterId ? updatedChapter : c).sort((a,b) => a.order - b.order));
      showToast("Resource added", "success");
      
      // Reset form
      setResourceForm({ title: "", type: "PDF", url: "", isPremium: false });
  ...
  const handleDeleteResource = async (chapterId: string, resourceIndex: number) => {
    ...
    try {
      ...
      const updatedResources = [...(chapter.resources || [])];
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 animate-fade-in space-y-8">
              ...
              return (
                <div key={chapter._id} className="bg-zinc-900 border border-white/5 rounded-xl overflow-hidden shadow-lg">
                  ...
                  <div className="p-2 space-y-1">
                    ...
                    {/* Inline Resources List */}
                    {chapter.resources && chapter.resources.length > 0 && (
                      <div className="mt-2 pt-2 border-t border-white/5 space-y-1">
                        <div className="px-3 py-1.5 text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                          <BookOpen className="w-3 h-3" /> Study Materials
                        </div>
                        {chapter.resources.map((res: any, rIdx: number) => (
                          <div key={rIdx} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/[0.02] transition-colors group/res border border-transparent hover:border-white/5">
                            <div className="flex items-center gap-3">
                              <div className="text-zinc-500 group-hover/res:text-accent transition-colors">
                                {res.type === 'PDF' || res.type === 'Document' ? <FileText className="w-4 h-4" /> : <LinkIcon className="w-4 h-4" />}
                              </div>
                              <div>
                                <div className="text-sm text-zinc-200 group-hover/res:text-white font-medium flex items-center gap-2">
                                  {res.title}
                                  {res.isPremium && <span className="text-[9px] bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded border border-amber-500/30 uppercase font-bold tracking-wider">Premium</span>}
                                </div>
                            ...
                            <div className="flex items-center gap-2 opacity-0 group-hover/res:opacity-100 transition-all">
                              ...
                              <button
                                onClick={() => handleDeleteResource(chapter._id, rIdx)}
                                className="p-2 text-zinc-400 hover:text-red-400 rounded-lg hover:bg-red-400/10"
                    ...
                    {/* Action Buttons */}
                    <div className="p-2 border-t border-white/5 mt-2 pt-3 flex items-center gap-3">
                      <Link
                      ...
                      <button
                        onClick={() => setManagingResourcesFor(chapter._id)}
                        className="flex items-center gap-2 text-xs font-medium text-zinc-400 hover:text-accent px-3 py-2 rounded-lg hover:bg-accent/10 transition-colors w-max"
                      >
      ...
      {managingResourcesFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#0A0A0A] border border-white/10 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
            <div className="p-6 border-b border-white/10 flex items-center justify-between shrink-0">
              ...
              <button 
                onClick={() => setManagingResourcesFor(null)}
                className="text-zinc-500 hover:text-white transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <form onSubmit={(e) => handleAddResource(e, managingResourcesFor)} className="bg-zinc-900/50 border border-white/5 p-5 rounded-xl space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="col-span-2">
                    <label className="text-xs text-zinc-500 block mb-1.5">Resource Title *</label>
                    <input
                      type="text"
                      value={resourceForm.title}
                      onChange={(e) => setResourceForm(p => ({ ...p, title: e.target.value }))}
                      className="w-full bg-black/40 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white focus:border-accent/50 outline-none"
                      placeholder="e.g. Arrays Cheat Sheet"
                      required
                    />
                  </div>
                  <div>
                ...
                <div>
                  <label className="text-xs text-zinc-500 block mb-1.5">Upload File OR Paste URL *</label>
                  <div className="flex gap-3 items-center">
                    <input
                      type="file"
                      onChange={(e) => setResourceFile(e.target.files?.[0] || null)}
                      className="flex-1 block w-full text-xs text-zinc-400
                    ...
                    <input
                      ...
                      className="flex-1 bg-black/40 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white focus:border-accent/50 outline-none"
                      placeholder="https://..."
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <input
                    ...
                    onChange={(e) => setResourceForm(p => ({ ...p, isPremium: e.target.checked }))}
                    className="rounded border-zinc-700 bg-black/40 text-accent focus:ring-accent/50 focus:ring-offset-0"
                  />
                  <label htmlFor="isPremium" className="text-xs text-zinc-400 cursor-pointer">
                    Mark as Premium (lock for non-paid users)
                  </label>
                </div>
                ...
                <div className="pt-2 border-t border-white/5 flex justify-end">
                  <button
                    ...
                    className="bg-accent text-black px-4 py-2 rounded-lg text-sm font-semibold hover:bg-accent/90 transition-colors flex items-center gap-2 disabled:opacity-50"
                  >
